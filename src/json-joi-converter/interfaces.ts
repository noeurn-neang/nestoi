...
export interface AnySchema<T = any> {
  ...
  fork?: {
    key:
      | string
      | ArrayMinOneItem<string>
      | ArrayMinOneItem<ArrayMinOneItem<string>>;
    adjuster?: Schema;
  };
  id?: string;
...
export interface StringSchema extends AnySchema<string> {
  ...
  uri?:
    | boolean
    | (Omit<UriOptions, 'scheme'> & { scheme?: string | JsonRegex });
  uuid?: boolean | GuidOptions;
}

type Peers =
  | ArrayMinOneItem<string | HierarchySeparatorOptions>
  | {
